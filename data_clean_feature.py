...
OUTPUT_PATH = "data/clean_features.csv"
...
for col in cols_medium_missing:
    ...
    if pd.isna(df[col].iloc[0]): df[col].iloc[0] = df[col].median()
    if pd.isna(df[col].iloc[-1]): df[col].iloc[-1] = df[col].median()

cols_numeric_min_missing = ['temperature_2m','relative_humidity_2m','wind_speed_10m','pressure_msl','precipitation','cloudcover']
for col in cols_numeric_min_missing:
    df[col] = df[col].fillna(df[col].median())

# Cap outliers (IQR)
numeric_cols = ['pm10','pm2_5','carbon_monoxide','nitrogen_dioxide','sulphur_dioxide','ozone',
                'temperature_2m','relative_humidity_2m','wind_speed_10m','pressure_msl','precipitation','cloudcover']
...
def calculate_aqi(pm25, pm10):
    ...
    aqi_pm25 = next((aqi_subindex(pm25,Bp_lo,Bp_hi,I_lo,I_hi) for Bp_lo,Bp_hi,I_lo,I_hi in pm25_breakpoints if Bp_lo<=pm25<=Bp_hi), None)
    aqi_pm10 = next((aqi_subindex(pm10,Bp_lo,Bp_hi,I_lo,I_hi) for Bp_lo,Bp_hi,I_lo,I_hi in pm10_breakpoints if Bp_lo<=pm10<=Bp_hi), None)

    if aqi_pm25 is not None and aqi_pm10 is not None: return max(aqi_pm25, aqi_pm10)
    elif aqi_pm25 is not None: return aqi_pm25
    elif aqi_pm10 is not None: return aqi_pm10
    else: return None
...
logging.info(f"✅ Cleaned and feature-engineered data saved at {OUTPUT_PATH}")
